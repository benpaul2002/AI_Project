...
#     threshold="1.5*median",  # More conservative feature selection
#     max_features=150
# )
# X_train_sel = selector.fit_transform(X_train, y_train)
# X_test_sel = selector.transform(X_test)
...
#     lgb_param, n_iter=30, cv=5, scoring='f1', n_jobs=-1, random_state=42
# )
...
#     cb_param, n_iter=30, cv=5, scoring='f1', n_jobs=-1, random_state=42
# )
# cb_search.fit(X_train, y_train)
# cb_best = cb_search.best_estimator_

# sel_rf  = SelectFromModel(rf_best,  threshold="1.25*median", max_features=150, prefit=True)  # ← Add prefit=True
# sel_lgb = SelectFromModel(lgb_best, threshold="1.25*median", max_features=150, prefit=True)
# sel_cb  = SelectFromModel(cb_best,  threshold="1.25*median", max_features=150, prefit=True)

# # 2. Build pipelines: selector → model
# rf_pipe  = Pipeline([("select", sel_rf),  ("model", rf_best)])
# lgb_pipe = Pipeline([("select", sel_lgb), ("model", lgb_best)])
# cb_pipe  = Pipeline([("select", sel_cb),  ("model", cb_best)])
...
# # 4. Train & evaluate on full feature set
